...
const KompetensiAddComponent: React.FC<KompetensiAddComponentProps> = ({
  sekolahId,
  onSuccess,
}) => {
      ...
      const fetchOkupasiByKode = async () => {
        try {
          ...
          setUnitKompetensiOptions(
            data.data.unit_kompetensi.map((unit: any) => ({
              value: unit.id,
              label: unit.nama,
            })),
          );
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!selectedOkupasi || selectedUnits.length === 0) {
      toast.error(
        'Both Okupasi and at least one Unit Kompetensi must be selected.',
        {
          position: 'bottom-right',
        },
      );
    ...
    try {
      await addKompetensi(
        sekolahId,
        selectedOkupasi.value,
        selectedUnits.map((unit: any) => ({ id: unit.value })),
      );
  ...
  return (
    <div>
      <form
        ...
        className='mb-6 p-4 bg-white rounded-lg shadow-lg dark:bg-gray-800 dark:text-white'
      >
